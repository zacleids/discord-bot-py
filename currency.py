...
def fetch_and_store_rates() -> dict:
    ...
    ray_id = get_ray_id()
    log_event("CURRENCY_API_REQUEST", {
        "event": "CURRENCY_API_REQUEST",
        "url": url,
    ...
    if data["result"] != "success":
        log_event("CURRENCY_API_ERROR", {
            "event": "CURRENCY_API_ERROR",
            "url": url,
            "base_currency": BASE_CURRENCY,
            "response": data,
            "ray_id": ray_id
    ...
    log_event("CURRENCY_API_SUCCESS", {
...
def get_rates() -> dict:
    ...
    ray_id = get_ray_id()
    if not rate:
        log_event("CURRENCY_CACHE_MISS", {
            "event": "CURRENCY_CACHE_MISS",
            "base_currency": BASE_CURRENCY,
            "ray_id": ray_id
        }, level="info")
    ...
    if age_seconds > REFRESH_HOURS * 3600:
        log_event("CURRENCY_CACHE_EXPIRED", {
...
def convert_currency(from_currency: str, to_currency: str, amount: float) -> float:
    ...
    ray_id = get_ray_id()
    log_event("CURRENCY_CONVERT", {
        "event": "CURRENCY_CONVERT",
    ...
    if from_currency not in rates or to_currency not in rates:
        log_event("CURRENCY_CONVERT_ERROR", {
    ...
    log_event("CURRENCY_CONVERT_RESULT", {
